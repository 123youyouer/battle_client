...
void cell::init()
{
	...
	m_side_distsqu.resize(3);
	m_side_distsqu[SIDE_AB] = (m_A.m_x - m_B.m_x)*(m_A.m_x - m_B.m_x) + (m_A.m_y - m_B.m_y)*(m_A.m_y - m_B.m_y);
	m_side_distsqu[SIDE_BC] = (m_B.m_x - m_C.m_x)*(m_B.m_x - m_C.m_x) + (m_B.m_y - m_C.m_y)*(m_B.m_y - m_C.m_y);
	m_side_distsqu[SIDE_CA] = (m_A.m_x - m_C.m_x)*(m_A.m_x - m_C.m_x) + (m_A.m_y - m_C.m_y)*(m_A.m_y - m_C.m_y);
}
...
void cell::computeHeuristic(const vector2 &target)
{
	...
	m_h = (target.m_x - m_center.m_x)*(target.m_x - m_center.m_x) + (target.m_y - m_center.m_y)*(target.m_y - m_center.m_y);
}
