...
//寻路
bool navmesh::find_path(vector2 start, vector2 end, float unit_width, vector<vector2> &result)
{
	...
	{
		build_path(start, end, start_cell, end_cell, unit_width, result);
	}
...
//构建路径
bool navmesh::build_path(vector2 start, vector2 end, cell* start_cell, cell* end_cell, float unit_width, vector<vector2> &result)
{
	float unit_width_squ = 0;// unit_width*unit_width;
	...
	while (m_openlist.size() > 0)
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			if (cur_node->get_side_distsqu(i) < unit_width_squ)
			{
				continue;
			}
...
bool navmesh::get_path(vector2 start, vector2 end, cell* start_cell, vector<vector2> &result)
{
	...
	for (int i = 0; i < cur_cells.size(); ++i)
	{
		if (i == 0)
		{
			for (int j = 0; j < 4; ++j)
			{
				if (start_cell->getlink((etriangle_side)j) == cur_cells[i]->getindex())
				{
					result.push_back(start_cell->m_wall_midpoint[j]);
				}
		...
		else
		{
			for (int j = 0; j < 4; ++j)
			{
				if (cur_cells[i-1]->getlink((etriangle_side)j) == cur_cells[i]->getindex())
				{
					result.push_back(cur_cells[i-1]->m_wall_midpoint[j]);
				}

			}
		}
		//result.push_back(cur_cells[i]->get_center());
