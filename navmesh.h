...
public:
	//寻路
	bool find_path(vector2 start, vector2 end, float unit_width, vector<vector2> &result);

private:
	//构建路径
	bool build_path(vector2 start, vector2 end, cell* start_cell, cell* end_cell, float unit_width, vector<vector2> &result);

	//构建最短路径
	bool get_path(vector2 start, vector2 end, vector<vector2> &result);

	bool get_path(vector2 start, vector2 end, cell* start_cell, vector<vector2> &result);
