...
bool delaunay::findDT(const line2D &line, vector2 &result)
{
	...
	while (loop)
	{
		...
		rectangle rect(cir);
		float angle132 = fabs(line_angle(p1, p3, p2));	// ∠p1p3p2
		for (int i = 0; i < allpoint.size(); ++i)
		{
			...
			float a1 = fabs(line_angle(p1, allpoint[i], p2));
			if (a1 > angle132)
			{
				p3 = allpoint[i];
				loop = true;
...
void delaunay::circumCircle(const vector2 &p1, const vector2 &p2, const vector2 &p3, circle &cir)
{
	...
	if (fabs(p1.m_y - p2.m_y) < 0.001f && fabs(p2.m_y - p3.m_y) < 0.001f)
	{
		return;
	}
	...
	if (fabs(p2.m_y - p1.m_y) < 0.001f)
	{
		xc = (p2.m_x + p1.m_x) / 2.0;
		yc = m2 * (xc - mx2) + my2;
	}
	else if (fabs(p3.m_y - p2.m_y) < 0.001f)
	{
